...
startSolo = () => {
    ...
    display(["canvas"]);
    typewriter(["W,A,S,D OR ARROWS TO MOVE, Q,E,Z|O TO STRAFE, X|P FOR SPEED, C TO OPEN", "TAB SHOWS THE MAP, T TO WRITE (MULTIPLAYER), YOU CAN ALSO USE THE MOUSE"]);
    callMain(commonArgs);
};
                ...
                if (data.direction) {
                    if (data.distance > 20) {
                        ...
                        switch (data.direction.angle) {
                            case "left":
                                press_left = true;
                                if (data.angle.degree >= nw[0] && data.angle.degree <= nw[1]) press_up = true;
                                if (data.angle.degree >= sw[0] && data.angle.degree <= sw[1]) press_down = true;
                                break;
                            case "right":
                                press_right = true;
                                if (data.angle.degree >= ne[0] && data.angle.degree <= ne[1]) press_up = true;
                                if (data.angle.degree >= se[0] && data.angle.degree <= se[1]) press_down = true;
                                break;
                            case "up":
                                press_up = true;
                                if (data.angle.degree >= nw[0] && data.angle.degree <= nw[1]) press_left = true;
                                if (data.angle.degree >= ne[0] && data.angle.degree <= ne[1]) press_right = true;
                                break;
                            case "down":
                                press_down = true;
                                if (data.angle.degree >= sw[0] && data.angle.degree <= sw[1]) press_left = true;
                                if (data.angle.degree >= se[0] && data.angle.degree <= se[1]) press_right = true;
                                break;
                        }
...
if (hasWebAssembly()) {
    var Module = {
        ...
        postRun: () => {
            Array.from(document.querySelectorAll(".controls")).forEach(function (control) {
        ...
        print: (text) => {
            if (text.startsWith("doom: ")) {
                ...
                switch (parseInt(id)) {
                    ...
                    case 10: // game starts
                        msg = false;
                        typewriter(["MOVE = MOUSE, WSOP OR ARROWS, SHIFT = RUN, E = USE, AD = STRAFE (OR HOLD C)", "TAB = MAP, T = SAY, F = FULLSCREEN, LEFT MOUSE OR SPACE = FIRE"]);
                        if (room) {
                            // multiplayer?
    ...
    display(["logo", "text"]);
    display(["canvas"], "none");
    var t = '<h1 class="vspace">Your browser has no WebAssembly Support</h1>';
    t += '<h1 class="vspace">You need a modern browser to run this demo</h1>';
    write(t);
}
